window.addEventListener('load', function(ev) {
  ...
  xhr.open("GET", "utils/load_imgs.php", true);
...
function delete_img(img) {
  ...
  if (confirm("Are you sure you want to delete this image ?")) {
	...
	xhr.onreadystatechange = function() {
	  if (xhr.status == 200 && xhr.readyState == 4) {
		if (xhr.responseText !== "")
		  ...
		  if (document.getElementsByClassName("photos").length === 0) {
			...
			photo_default.setAttribute("src", "photos/icons/user.jpg");
	  ...
	  xhr.open("POST", "utils/delete_img.php", true);
	  xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
...
.catch(function(err){
  ...
  function save_and_display_photo(img_data, filter_data) {
	...
	if (img_data !== "" && filter_data !== "") {
	  ...
	  xhr.open("POST", "utils/store_img.php", true);
	  xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
