function	login_validate() {
	...
	((/^[a-zA-Z0-9_-]{2,20}$/).test(login) == false) || passwd_elem.validity.RangeUnderflow ||
...
function	register_validate() {
	...
	((/^[a-zA-Z0-9_-]{2,20}$/).test(login) == false))
		msg.innerHTML += "⚠️Username must be at least 2 characters. Allowed characters : a-z, A-Z, 0-9, '-' and '_'.<br/>";
	...
	if (msg.innerHTML !== "")
    	...
    	if (xhr.status == 200 && xhr.readyState == 4){
				...
				if (msg.innerHTML !== "")
					...
					validation_msg.innerHTML = "VERIFY YOUR EMAIL AT "+ email + " PLEASE.<br/>";
	...
	if (msg.innerHTML === "") {
	    ...
	    if (xhr.status == 200 && xhr.readyState == 4) {
				...
				if (msg.innerHTML !== "")
					...
					put_login_form();
					var validation_msg = document.getElementById("main_msg");
					validation_msg.innerHTML = "CHECK EMAIL FOR NEW PASSWORD.<br/>";
					validation_msg.style.display = "block";
				}
			}
		}
			xhr.open("POST", "utils/reset_pass.php", true);
			xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
