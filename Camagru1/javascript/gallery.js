...
function add_like(like_button) {
  ...
  xhr.open("POST", "../utils/add_like.php", true);
...
function send_comment(com_button) {
  ...
  xhr.open("POST", "../utils/send_com.php", true);
  xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
...
window.addEventListener('load', function(ev) {
  ...
  xhr.open("GET", "../utils/load_gallery.php?offset=0", true);
  xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  xhr.send();
});


document.getElementById("load_more").addEventListener('click', function(ev) {
  var nb_pub = document.querySelectorAll('main .publication').length,
      error_msg = document.getElementById("load_more_msg");
      publications_container = document.getElementById("publications_container"),
      xhr = new XMLHttpRequest();
  ...
  xhr.open("GET", "../utils/load_gallery.php?offset=" + nb_pub, true);
  xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  xhr.send();
});
